...
| 魚池名稱 | 說明 | 目前股票數 |
|---|---|---|
| 🔥 姊夫爆發小魚池 | **貴金屬 ETF 戰術池**（V9.3 起：動態篩選全數移除，改固定 4 檔 — 00635U 黃金期貨、00708L 黃金正2、00674R 黃金反1、00738U 白銀期貨；停損停利依商品分級：原型／反向 −7%/+8%、正2倍槓桿 −10%/+12%；訊號採技術雙確認） | 4 支（固定） |
| 🍁 楓大永動魚池 | 穩定動能股（2308、00923、00910、2327、1785、2344、6155、4532） | 8 支 |
...
Yahoo Finance 全市場批量粗篩（chunk=150）
  ↓ 🛡️ 空跑保護（V9.4）：有效股價 < 100 檔或覆蓋率 < 30% → 中止，不覆寫戰報、不推送
  ↓ 第一關：成交量 ≥ 2,000 張
  ↓ 第二關：收盤價 > MA30
...
> 🔥 姊夫爆發小魚池自 **V9.3 起不再參與上述流程**：改為 4 檔貴金屬期貨 ETF 固定池，直接進魚池監控段（每檔 1~2 次 FinMind API）。停損停利依商品槓桿分級、動作訊號改純技術雙確認（`close ≥ MA5` 且 `vol_ratio ≥ 1.0`），因期貨 ETF 法人買賣以自營商避險造市為主、沿用法人條件會失真；該池亦排除於全域多空比統計外（反向 ETF 的「買入加碼」語意與個股相反）。
...
| 目標價 | 收盤 × 1.5 |
| 停損價 | ATR×2 動態停損（護欄 −6%～−15%，V8.9 起取代固定 ×0.9；姊夫 ETF 池另用分級固定值 −7%／−10%） |
...
| **Grace** | 題材面研究員 | 🔥姊夫爆發（貴金屬 ETF）、🔭測試員觀察 | 市場熱點、產業趨勢、短線催化劑分析 | `Grace_SKILL.md` |
| **Joe** | 技術面研究員 | 🌊汪洋大魚、🐅三日猛虎、🔥姊夫 ETF 池 | MA5/MA30 趨勢判讀、停利×1.5／ATR×2 動態停損、姊夫 ETF 池分級風控與技術訊號 | `Joe_SKILL.md` |
| **Eric** | 籌碼分析研究員 | 所有魚池（除汪洋大魚、姊夫 ETF 池） | 三大法人買賣超、融資券比率預警 | `Eric_SKILL.md` |

### 魚池 × 成員對應速查

| 魚池 | 主責研究員 |
|------|-----------|
| 🔥 姊夫爆發小魚池 | Joe（技術訊號與分級停損停利）、Grace（貴金屬題材）｜V9.3 起為 ETF 池，Eric 籌碼條件已移除 |
...
> 正本在工作區全域目錄 `C:\AIworkplace\.claude\skills\[skill 名]\SKILL.md`（非本 repo 內的 `skills/`）。**但該目錄在本倉庫開啟的工作階段不會被載入**（2026-09-12 實測：可用清單查無下列 4 個），故另複製一份至使用者層 `%USERPROFILE%\.claude\skills\` 供實際載入；**改正本後須同步使用者層副本**。
...
- 版本號格式：`V[主版本].[次版本]`（目前為 **V9.4**，前端 **V9.2 UI**）
