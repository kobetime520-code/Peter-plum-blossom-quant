...
| `radar/radar.py` | 核心雷達掃描程式（**靜水流深戰情室 V8.6**，正式版） |
| `radar test/radar.py` | 雷達沙盒（V8.6，與正式版同步） |
...
- **🆕 theme_tag**：Grace 產業題材標籤（11 大分類，零 API 消耗）
- **🆕 sweet_buy_low / sweet_buy_high**：Joe 甜蜜買進區間（max(MA5,MA10)×0.99 ~ ×1.005）
...
| 2026-05-16 | V8.6 | 新增 theme_tag（Grace 產業題材標籤）、sweet_buy_low/high（Joe 甜蜜買進區間）、first_target（第一停利+15%）；index.html 全能戰情室創意升級（粒子背景/神經網路SVG/打字機標語/公式橫幅/甜蜜點區塊/評分條/分級徽章）；CLAUDE.md 套入 Team Stock 10 位成員 SKILL 索引。 |
